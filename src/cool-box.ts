let computed_style: CSSStyleDeclaration;
const coolbox: HTMLElement | null = document.querySelector('.cool-box');
if (coolbox)
	computed_style = window.getComputedStyle(coolbox);

window.addEventListener('load', function() {
	for (let i = 1 ; i <= 5 ; i++) {
		let imp: NodeList = document.querySelectorAll('[importance="' + i + '"]');
		let k = 6 - i;
		imp.forEach((elem: Node) => {
